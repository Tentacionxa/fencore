...
void CastViewer::kickViewer(std::vector<std::string> list) {
	for (const auto &it : list) {
		for (const auto &sit : m_viewers) {
...
const std::vector<std::string> &CastViewer::muteCastList() const {
	return m_mutes;
}

void CastViewer::muteViewer(std::vector<std::string> mutes) {
	m_mutes = mutes;
}

const std::map<std::string, uint32_t> &CastViewer::banCastList() const {
	return m_bans;
}

void CastViewer::banViewer(std::vector<std::string> bans) {
...
void CastViewer::sendIconBakragore(const IconBakragore icon) {
	if (m_owner) {
		...
		for (const auto &it : m_viewers) {
			it.first->sendIconBakragore(icon);
		}
	}
}
...
void CastViewer::BestiarysendCharms() {
	if (m_owner) {
...
std::vector<std::string> CastViewer::listViewers() const {
