...
local config = {
    ...
    playerPositions = {
        { pos = Position(33078, 32333, 15), teleport = Position(33043, 32338, 15) },
        { pos = Position(33077, 32333, 15), teleport = Position(33043, 32338, 15) },
        { pos = Position(33076, 32333, 15), teleport = Position(33043, 32338, 15) },
        { pos = Position(33075, 32333, 15), teleport = Position(33043, 32338, 15) },
        { pos = Position(33074, 32333, 15), teleport = Position(33043, 32338, 15) },
    },
    specPos = {
        from = Position(33036, 32328, 15),
        to = Position(33052, 32345, 15),
    },
    teleportTimer = 1800, -- seconds, 30 minutes (1800s)
    cooldownTimer = 12 * 60 * 60, -- 12 hours in seconds
	storage = 80315 -- Needs to be unique, used for player cooldown
}

local function removeMonsterFromArea(fromPos, toPos)
    local exitPosition = Position(33076, 32331, 15)
            ...
            if creatures then -- make sure getCreatures doesn't return nil
                for _, creature in ipairs(creatures) do
                    if creature:isPlayer() then
                        ...
                        creature:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have 30 minutes to defeat the boss before you will be teleported out.")
                    end
                end
            end
        end
