...
local vampire = CreatureEvent("VampireDeath")
function vampire.onDeath(creature)
	local targetMonster = creature:getMonster()
	local position = targetMonster:getPosition()
	position:sendMagicEffect(CONST_ME_TELEPORT)
	local item = Game.createItem(1949, 1, position)
	local teleportToPosition = Position(32286, 31627, 6)
	...
	local spectators, spectator = Game.getSpectators(Position(32285, 31624, 6), false, false, 10, 10, 10, 10)
