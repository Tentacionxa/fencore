...
local hoursCooldown = 1 -- Quantidade de horas de cooldown
...
function blessItem.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if lastUse > currentTime then
        ...
        player:sendTextMessage(MESSAGE_LOOK, string.format("You can only use this every 1 hour. Time remaining: %02d:%02d:%02d.", hours, minutes, seconds))
        return true
    end
