...
public:
	...
	void kickViewer(std::vector<std::string> list);
	const std::vector<std::string> &muteCastList() const;
	void muteViewer(std::vector<std::string> _mutes);
	const std::map<std::string, uint32_t> &banCastList() const;
	void banViewer(std::vector<std::string> _bans);
	...
	void addViewer(std::shared_ptr<ProtocolGame> client, std::string name = "", bool spy = false);
