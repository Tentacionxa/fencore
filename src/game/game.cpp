...
namespace InternalGame {
  void sendBlockEffect(BlockType_t blockType, CombatType_t combatType,
    const Position & targetPos, std::shared_ptr < Creature > source) {
  ...
  bool playerCanUseItemWithOnHouseTile(std::shared_ptr < Player > player, std::shared_ptr < Item > item,
    const Position & toPos, int toStackPos, int toItemId) {
    if (!player || !item) {
      return false;
    }
  ...
  template < typename T >
    T getCustomAttributeValue(std::shared_ptr < Item > item,
...
Game::Game() {
  ...
  m_highscoreCategoriesNames = {
    ...
    {
      static_cast < uint8_t > (HighscoreCategories_t::AXE_FIGHTING),
      "Axe Fighting"
    },
    {
      static_cast < uint8_t > (HighscoreCategories_t::BOSS_POINTS),
      "Boss Points"
    },
    {
      static_cast < uint8_t > (HighscoreCategories_t::CHARMS),
      "Charm Points"
    },
    {
    ...
    {
      static_cast < uint8_t > (HighscoreCategories_t::DISTANCE_FIGHTING),
      "Distance Fighting"
    },
    {
      static_cast < uint8_t > (HighscoreCategories_t::DROME),
      "Drome Score"
    },
    {
    ...
    {
      static_cast < uint8_t > (HighscoreCategories_t::FISHING),
      "Fishing"
    },
    {
      static_cast < uint8_t > (HighscoreCategories_t::FIST_FIGHTING),
      "Fist Fighting"
    },
    {
      static_cast < uint8_t > (HighscoreCategories_t::GOSHNAR),
      "Goshnar's Taint"
    },
    {
      static_cast < uint8_t > (HighscoreCategories_t::LOYALTY),
      "Loyalty Points"
    },
    {
      static_cast < uint8_t > (HighscoreCategories_t::MAGIC_LEVEL),
      "Magic Level"
    },
    {
    ...
    {
      static_cast < uint8_t > (HighscoreCategories_t::SWORD_FIGHTING),
      "Sword Fighting"
    },
  };
  ...
  m_summaryCategories = {
    ...
    {
      static_cast < uint8_t > (Summary_t::BOOSTS),
      "xp-boosts"
    },
    {
      static_cast < uint8_t > (Summary_t::PREY_CARDS),
      "prey-cards"
    },
    {
      static_cast < uint8_t > (Summary_t::BLESSINGS),
      "blessings"
    },
    {
      static_cast < uint8_t > (Summary_t::INSTANT_REWARDS),
      "instant-rewards"
    },
    {
      static_cast < uint8_t > (Summary_t::HIRELINGS),
      "hirelings"
    },
  };
...
void Game::start(ServiceManager * manager) {
  ...
  g_dispatcher().cycleEvent(
    EVENT_LIGHTINTERVAL_MS, [this] {
      checkLight();
    }, "Game::checkLight"
  );
  g_dispatcher().cycleEvent(
    EVENT_CHECK_CREATURE_INTERVAL, [this] {
      checkCreatures();
    }, "Game::checkCreatures"
  );
  g_dispatcher().cycleEvent(
    EVENT_IMBUEMENT_INTERVAL, [this] {
      checkImbuements();
    }, "Game::checkImbuements"
  );
  g_dispatcher().cycleEvent(
    EVENT_LUA_GARBAGE_COLLECTION, [this] {
  ...
  if (marketItemsPriceIntervalMinutes > 0) {
    ...
    g_dispatcher().cycleEvent(
      marketItemsPriceIntervalMS, [this] {
        loadItemsPrice();
      }, "Game::loadItemsPrice"
    );
  }
...
std::shared_ptr < Cylinder > Game::internalGetCylinder(std::shared_ptr < Player > player,
  const Position & pos) {
  if (pos.x != 0xFFFF) {
...
std::shared_ptr < Thing > Game::internalGetThing(std::shared_ptr < Player > player,
  const Position & pos, int32_t index, uint32_t itemId, StackPosType_t type) {
  if (pos.x != 0xFFFF) {
...
bool Game::internalPlaceCreature(std::shared_ptr < Creature > creature,
  const Position & pos, bool extendedPos /*=false*/ , bool forced /*= false*/ , bool creatureCheck /*= false*/ ) {
...
bool Game::placeCreature(std::shared_ptr < Creature > creature,
  const Position & pos, bool extendedPos /*=false*/ , bool forced /*= false*/ ) {
...
void Game::playerInspectItem(std::shared_ptr < Player > player,
  const Position & pos) {
  metrics::method_latency measure(__METHOD_NAME__);
...
void Game::playerMoveThing(uint32_t playerId,
  const Position & fromPos, uint16_t itemId, uint8_t fromStackPos,
...
void Game::playerMoveCreatureByID(uint32_t playerId, uint32_t movingCreatureId,
  const Position & movingCreatureOrigPos,
...
void Game::playerMoveCreature(std::shared_ptr < Player > player, std::shared_ptr < Creature > movingCreature,
  ...
  if (!player -> canDoAction()) {
    const auto & task = createPlayerTask(
      600, [this, player, movingCreature, toTile, movingCreatureOrigPos] {
...
ReturnValue Game::internalMoveCreature(const std::shared_ptr < Creature > & creature,
  const std::shared_ptr < Tile > & toTile, uint32_t flags /*= 0*/ ) {
  metrics::method_latency measure(__METHOD_NAME__);
...
void Game::playerMoveItemByPlayerID(uint32_t playerId,
  const Position & fromPos, uint16_t itemId, uint8_t fromStackPos,
    const Position & toPos, uint8_t count) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
...
void Game::playerMoveItem(std::shared_ptr < Player > player,
  const Position & fromPos, uint16_t itemId, uint8_t fromStackPos,
    const Position & toPos, uint8_t count, std::shared_ptr < Item > item, std::shared_ptr < Cylinder > toCylinder) {
  if (!player -> canDoAction()) {
...
std::tuple < ReturnValue, uint32_t, uint32_t > Game::addItemBatch(const std::shared_ptr < Cylinder > & toCylinder,
  const std::vector < std::shared_ptr < Item >> & items, uint32_t flags /* = 0 */ , bool dropOnMap /* = true */ , uint32_t autoContainerId /* = 0 */ ) {
...
std::tuple < ReturnValue, uint32_t, uint32_t > Game::createItemBatch(const std::shared_ptr < Cylinder > & toCylinder,
  const std::vector < std::tuple < uint16_t, uint32_t, uint16_t >> & itemCounts, uint32_t flags /* = 0 */ , bool dropOnMap /* = true */ , uint32_t autoContainerId /* = 0 */ ) {
  metrics::method_latency measure(__METHOD_NAME__);
...
std::tuple < ReturnValue, uint32_t, uint32_t > Game::createItem(const std::shared_ptr < Cylinder > & toCylinder, uint16_t itemId, uint32_t count, uint16_t subType, uint32_t flags /* = 0 */ , bool dropOnMap /* = true */ , uint32_t autoContainerId /* = 0 */ ) {
  return createItemBatch(toCylinder, {
    std::make_tuple(itemId, count, subType)
  }, flags, dropOnMap, autoContainerId);
}
...
ReturnValue Game::internalTeleport(const std::shared_ptr < Thing > & thing,
  const Position & newPos, bool pushMove /* = true*/ , uint32_t flags /*= 0*/ ) {
  metrics::method_latency measure(__METHOD_NAME__);
  if (thing == nullptr) {
...
bool Game::handleFallbackLogic(std::shared_ptr < Player > player, std::shared_ptr < Container > & lootContainer, ContainerIterator & containerIterator,
  const bool & fallbackConsumed) {
...
void Game::playerMove(uint32_t playerId, Direction direction) {
  ...
  player -> startAutoWalk(std::vector < Direction > {
    direction
  }, false);
}
...
bool Game::playerBroadcastMessage(std::shared_ptr < Player > player,
...
void Game::playerChannelInvite(uint32_t playerId,
...
void Game::playerChannelExclude(uint32_t playerId,
  const std::string & name) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
    return;
...
void Game::playerAutoWalk(uint32_t playerId,
  const std::vector < Direction > & listDir) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
    return;
  }
...
void Game::forcePlayerAutoWalk(uint32_t playerId,
  const std::vector < Direction > & listDir) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
    return;
...
void Game::playerUseItemEx(uint32_t playerId,
  const Position & fromPos, uint8_t fromStackPos, uint16_t fromItemId,
  ...
  if (ret != RETURNVALUE_NOERROR) {
    if (ret == RETURNVALUE_TOOFARAWAY) {
      ...
      if (player -> getPathTo(walkToPos, listDir, 0, 1, true, true)) {
        ...
        std::shared_ptr < Task > task = createPlayerTask(
          400, [this, playerId, itemPos, itemStackPos, fromItemId, toPos, toStackPos, toItemId] {
  ...
  if (!canDoAction) {
    ...
    std::shared_ptr < Task > task = createPlayerTask(
      delay, [this, playerId, fromPos, fromStackPos, fromItemId, toPos, toStackPos, toItemId] {
  ...
  if (player -> isDepotSearchOpenOnItem(fromItemId)) {
    if (item -> isInsideDepot(true)) {
      ...
      if (auto targetThing = internalGetThing(player, toPos, toStackPos, toItemId, STACKPOS_FIND_THING); targetThing && targetThing -> getItem() && targetThing -> getItem() -> isInsideDepot(true)) {
        mustReloadDepotSearch = true;
      }
    }
  }
...
void Game::playerUseItem(uint32_t playerId,
  const Position & pos, uint8_t stackPos, uint8_t index, uint16_t itemId) {
  metrics::method_latency measure(__METHOD_NAME__);
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
    return;
  ...
  if (ret != RETURNVALUE_NOERROR) {
    if (ret == RETURNVALUE_TOOFARAWAY) {
      ...
      if (player -> getPathTo(pos, listDir, 0, 1, true, true)) {
        ...
        std::shared_ptr < Task > task = createPlayerTask(
          400, [this, playerId, pos, stackPos, index, itemId] {
  ...
  if (!canDoAction) {
    ...
    std::shared_ptr < Task > task = createPlayerTask(
      delay, [this, playerId, pos, stackPos, index, itemId] {
        playerUseItem(playerId, pos, stackPos, index, itemId);
      }, "Game::playerUseItem"
    );
    if (it.isRune() || it.type == ITEM_TYPE_POTION) {
...
void Game::playerUseWithCreature(uint32_t playerId,
  const Position & fromPos, uint8_t fromStackPos, uint32_t creatureId, uint16_t itemId) {
  metrics::method_latency measure(__METHOD_NAME__);
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
    return;
  ...
  if (!canDoAction) {
    ...
    std::shared_ptr < Task > task = createPlayerTask(
      delay, [this, playerId, fromPos, fromStackPos, creatureId, itemId] {
...
void Game::playerRotateItem(uint32_t playerId,
  const Position & pos, uint8_t stackPos,
...
void Game::playerConfigureShowOffSocket(uint32_t playerId,
  const Position & pos, uint8_t stackPos,
    const uint16_t itemId) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
    ...
    if (player -> getPathTo(pos, listDir, 0, 1, true, false)) {
      ...
      if (isPodiumOfRenown) {
        task = createPlayerTask(
          400, [player, item, pos, itemId, stackPos] {
            player -> sendPodiumWindow(item, pos, itemId, stackPos);
          }, "Game::playerConfigureShowOffSocket"
        );
      } else {
        task = createPlayerTask(
          400, [player, item, pos, itemId, stackPos] {
...
void Game::playerSetShowOffSocket(uint32_t playerId, Outfit_t & outfit,
  const Position & pos, uint8_t stackPos,
    const uint16_t itemId, uint8_t podiumVisible, uint8_t direction) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player || pos.x == 0xFFFF) {
    return;
  }
  ...
  if (!Position::areInRange < 1, 1, 0 > (pos, player -> getPosition())) {
    ...
    if (player -> getPathTo(pos, listDir, 0, 1, true, false)) {
      ...
      std::shared_ptr < Task > task = createPlayerTask(
        400, [this, playerId, pos] {
          playerBrowseField(playerId, pos);
        }, "Game::playerBrowseField"
      );
      player -> setNextWalkActionTask(task);
    } else {
      player -> sendCancelMessage(RETURNVALUE_THEREISNOWAY);
...
void Game::playerWrapableItem(uint32_t playerId,
  const Position & pos, uint8_t stackPos,
    const uint16_t itemId) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
...
void Game::playerWriteItem(uint32_t playerId, uint32_t windowTextId,
...
void Game::playerBrowseField(uint32_t playerId,
  const Position & pos) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
    return;
...
void Game::playerStowItem(uint32_t playerId,
  const Position & pos, uint16_t itemId, uint8_t stackpos, uint8_t count, bool allItems) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
    return;
  }
...
void Game::playerUpdateHouseWindow(uint32_t playerId, uint8_t listId, uint32_t windowTextId,
  const std::string & text) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
    return;
...
void Game::playerRequestTrade(uint32_t playerId,
  const Position & pos, uint8_t stackPos, uint32_t tradePlayerId, uint16_t itemId) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
    return;
  }
  ...
  if (!Position::areInRange < 1, 1 > (tradeItemPosition, playerPosition)) {
    ...
    if (player -> getPathTo(pos, listDir, 0, 1, true, true)) {
      ...
      std::shared_ptr < Task > task = createPlayerTask(
        400, [this, playerId, pos, stackPos, tradePlayerId, itemId] {
          playerRequestTrade(playerId, pos, stackPos, tradePlayerId, itemId);
        }, "Game::playerRequestTrade"
      );
      player -> setNextWalkActionTask(task);
    } else {
      player -> sendCancelMessage(RETURNVALUE_THEREISNOWAY);
...
void Game::playerLookAt(uint32_t playerId, uint16_t itemId,
  const Position & pos, uint8_t stackPos) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
    return;
...
void Game::playerQuickLoot(uint32_t playerId,
  const Position & pos, uint16_t itemId, uint8_t stackPos, std::shared_ptr < Item > defaultItem, bool lootAllCorpses, bool autoLoot) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
    return;
  }
...
void Game::playerLootAllCorpses(std::shared_ptr < Player > player,
  const Position & pos, bool lootAllCorpses, bool autoloot = false) {
  if (lootAllCorpses) {
    if (autoloot) {
      auto playerPosition = Position(player -> getPosition().x, player -> getPosition().y, player -> getPosition().z);
      playerLootAllCorpses(player, playerPosition, lootAllCorpses);

      playerPosition = Position(player -> getPosition().x + 1, player -> getPosition().y, player -> getPosition().z);
      playerLootAllCorpses(player, playerPosition, lootAllCorpses);

      playerPosition = Position(player -> getPosition().x + 1, player -> getPosition().y + 1, player -> getPosition().z);
      playerLootAllCorpses(player, playerPosition, lootAllCorpses);

      playerPosition = Position(player -> getPosition().x + 1, player -> getPosition().y - 1, player -> getPosition().z);
      playerLootAllCorpses(player, playerPosition, lootAllCorpses);

      playerPosition = Position(player -> getPosition().x, player -> getPosition().y + 1, player -> getPosition().z);
      playerLootAllCorpses(player, playerPosition, lootAllCorpses);
...
void Game::playerSetManagedContainer(uint32_t playerId, ObjectCategory_t category,
  const Position & pos, uint16_t itemId, uint8_t stackPos, bool isLootContainer) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player || pos.x != 0xffff) {
    return;
  }
...
void Game::playerQuickLootBlackWhitelist(uint32_t playerId, QuickLootFilter_t filter,
  const std::vector < uint16_t > itemIds) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
    return;
  }
...
void Game::playerRequestOpenContainerFromDepotSearch(uint32_t playerId,
  const Position & pos) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player || !player -> isDepotSearchOpen()) {
    return;
...
void Game::playerSetAttackedCreature(uint32_t playerId, uint32_t creatureId) {
  ...
  g_dispatcher().addEvent([this, plyerId = player -> getID()] {
    updateCreatureWalk(plyerId);
...
void Game::playerRequestAddVip(uint32_t playerId,
  const std::string & name) {
  if (name.length() > 25) {
    return;
  }
...
void Game::playerRequestEditVip(uint32_t playerId, uint32_t guid,
  const std::string & description, uint32_t icon, bool notify, std::vector < uint8_t > vipGroupsId) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
    return;
  }
...
bool Game::playerSaySpell(std::shared_ptr < Player > player, SpeakClasses type,
  const std::string & text) {
  if (player -> walkExhausted()) {
    return true;
  }
...
void Game::playerWhisper(std::shared_ptr < Player > player,
  const std::string & text) {
...
bool Game::playerYell(std::shared_ptr < Player > player,
  const std::string & text) {
...
bool Game::playerSpeakTo(std::shared_ptr < Player > player, SpeakClasses type,
  const std::string & receiver,
    const std::string & text) {
...
void Game::playerSpeakToNpc(std::shared_ptr < Player > player,
  const std::string & text) {
  if (player == nullptr) {
...
//--
bool Game::canThrowObjectTo(const Position & fromPos,
  const Position & toPos,
    const SightLines_t lineOfSight /*= SightLine_CheckSightLine*/ ,
      const int32_t rangex /*= Map::maxClientViewportX*/ ,
        const int32_t rangey /*= Map::maxClientViewportY*/ ) {
  return map.canThrowObjectTo(fromPos, toPos, lineOfSight, rangex, rangey);
}

bool Game::isSightClear(const Position & fromPos,
  const Position & toPos, bool floorCheck) {
...
bool Game::internalCreatureSay(std::shared_ptr < Creature > creature, SpeakClasses type,
  const std::string & text, bool ghostMode, Spectators * spectatorsPtr /* = nullptr*/ ,
...
void Game::internalCreatureChangeOutfit(std::shared_ptr < Creature > creature,
...
void Game::notifySpectators(const CreatureVector & spectators,
  const Position & targetPos, std::shared_ptr < Player > attackerPlayer, std::shared_ptr < Monster > targetMonster) {
  if (!spectators.empty()) {
    for (const auto & spectator: spectators) {
      if (!spectator) {
        continue;
...
bool Game::combatChangeHealth(std::shared_ptr < Creature > attacker, std::shared_ptr < Creature > target, CombatDamage & damage, bool isEvent /*= false*/ ) {
  ...
  if (damage.primary.value > 0) {
    ...
    if (!damage.extension && attackerMonster && targetPlayer) {
      // Charm rune (target as player)
      if (charmRune_t activeCharm = g_iobestiary().getCharmFromTarget(targetPlayer, g_monsters().getMonsterTypeByRaceId(attackerMonster -> getRaceId())); activeCharm != CHARM_NONE && activeCharm != CHARM_CLEANSE) {
        if (const auto charm = g_iobestiary().getBestiaryCharm(activeCharm); charm -> type == CHARM_DEFENSIVE && charm -> chance > normal_random(0, 100) && g_iobestiary().parseCharmCombat(charm, targetPlayer, attacker, (damage.primary.value + damage.secondary.value))) {
...
void Game::sendDamageMessageAndEffects(
  std::shared_ptr < Creature > attacker, std::shared_ptr < Creature > target,
...
void Game::buildMessageAsTarget(
  ...
  const CombatDamage & damage, std::shared_ptr < Player > attackerPlayer,
    std::shared_ptr < Player > targetPlayer, TextMessage & message, std::stringstream & ss,
...
void Game::buildMessageAsAttacker(
  ...
  const CombatDamage & damage, TextMessage & message,
    std::stringstream & ss,
    const std::string & damageString
) const {
  ss.str({});
...
void Game::sendEffects(
  std::shared_ptr < Creature > target,
  const CombatDamage & damage,
    const Position & targetPos, TextMessage & message,
...
void Game::applyCharmRune(
  std::shared_ptr < Monster > targetMonster, std::shared_ptr < Player > attackerPlayer, std::shared_ptr < Creature > target,
...
void Game::applyManaLeech(
  std::shared_ptr < Player > attackerPlayer, std::shared_ptr < Monster > targetMonster, std::shared_ptr < Creature > target,
  const CombatDamage & damage,
    const int32_t & realDamage
) const {
...
int32_t Game::calculateLeechAmount(const int32_t & realDamage,
  const uint16_t & skillAmount, int targetsAffected) const {
...
void Game::removeMagicEffect(const CreatureVector & spectators,
  const Position & pos, uint16_t effect) {
  for (const auto & spectator: spectators) {
    if (const auto & tmpPlayer = spectator -> getPlayer()) {
...
void Game::cacheQueryHighscore(const std::string & key,
  const std::string & query, uint32_t page, uint8_t entriesPerPage) {
  QueryHighscoreCacheEntry queryEntry {
    query,
    page,
    entriesPerPage,
...
void Game::playerHighscores(std::shared_ptr < Player > player, HighscoreType_t type, uint8_t category, uint32_t vocation,
  const std::string & , uint16_t page, uint8_t entriesPerPage) {
...
void Game::playerReportRuleViolationReport(uint32_t playerId,
  const std::string & targetName, uint8_t reportType, uint8_t reportReason,
...
void Game::playerReportBug(uint32_t playerId,
...
void Game::playerDebugAssert(uint32_t playerId,
  const std::string & assertLine,
...
namespace {
  bool removeOfferItems(const std::shared_ptr < Player > & player,
    const std::shared_ptr < DepotLocker > & depotLocker,
      const ItemType & itemType, uint16_t amount, uint8_t tier, std::ostringstream & offerStatus) {
...
bool checkCanInitCreateMarketOffer(std::shared_ptr < Player > player, uint8_t type,
  const ItemType & it, uint16_t amount, uint64_t price, std::ostringstream & offerStatus) {
  if (!player) {
...
void Game::playerCreateMarketOffer(uint32_t playerId, uint8_t type, uint16_t itemId, uint16_t amount, uint64_t price, uint8_t tier, bool anonymous) {
  ...
  if (type == MARKETACTION_SELL) {
    ...
    g_metrics().addCounter("balance_decrease", fee, {
      {
        "player",
        player -> getName()
      },
    ...
    g_metrics().addCounter("balance_decrease", totalPrice, {
      {
        "player",
        player -> getName()
      },
...
void Game::playerCancelMarketOffer(uint32_t playerId, uint32_t timestamp, uint16_t counter) {
  ...
  if (offer.type == MARKETACTION_BUY) {
    ...
    g_metrics().addCounter("balance_decrease", offer.price * offer.amount, {
      {
        "player",
        player -> getName()
      },
...
void Game::parsePlayerExtendedOpcode(uint32_t playerId, uint8_t opcode,
...
void Game::playerBosstiarySlot(uint32_t playerId, uint8_t slotId, uint32_t selectedBossId) {
  ...
  if (uint32_t boostedBossId = g_ioBosstiary().getBoostedBossId(); selectedBossId == 0 && bossIdSlot != boostedBossId) {
    ...
    g_metrics().addCounter("balance_decrease", removePrice, {
      {
        "player",
        player -> getName()
      },
...
void Game::playerSetMonsterPodium(uint32_t playerId, uint32_t monsterRaceId,
  const Position & pos, uint8_t stackPos,
    const uint16_t itemId, uint8_t direction,
      const std::pair < uint8_t, uint8_t > & podiumAndMonsterVisible) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player || pos.x == 0xFFFF) {
    return;
  }
  ...
  if (!Position::areInRange < 1, 1, 0 > (pos, player -> getPosition())) {
    if (std::vector < Direction > listDir; player -> getPathTo(pos, listDir, 0, 1, true, false)) {
      g_dispatcher().addEvent([this, playerId = player -> getID(), listDir] {
        playerAutoWalk(playerId, listDir);
      }, "Game::playerAutoWalk");
      std::shared_ptr < Task > task = createPlayerTask(
...
void Game::playerRotatePodium(uint32_t playerId,
  const Position & pos, uint8_t stackPos,
    const uint16_t itemId) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
...
void Game::playerWheelGemAction(uint32_t playerId, NetworkMessage & msg) {
  ...
  case WheelGemAction_t::ToggleLock:
    ...
    break;
  case WheelGemAction_t::EnhanceMod:
    posMod = msg.get < uint8_t > ();
    player -> wheel() -> enhanceMod(param, posMod);
    break;
  default:
...
uint32_t Game::makeFiendishMonster(uint32_t forgeableMonsterId /* = 0*/ , bool createForgeableMonsters /* = false*/ ) {
  ...
  if (monster && monster -> canBeForgeMonster()) {
    ...
    auto schedulerTask = createPlayerTask(
      finalTime,
      [this, monster] {
        updateFiendishMonsterStatus(monster -> getID(), monster -> getName());
      },
...
void Game::playerCheckActivity(const std::string & playerName, int interval) {
  ...
  g_dispatcher().scheduleEvent(
    1000, [this, playerName, interval] {
      playerCheckActivity(playerName, interval);
    }, "Game::playerCheckActivity"
  );
}

void Game::playerRewardChestCollect(uint32_t playerId,
  const Position & pos, uint16_t itemId, uint8_t stackPos, uint32_t maxMoveItems /* = 0*/ ) {
  std::shared_ptr < Player > player = getPlayerByID(playerId);
  if (!player) {
    return;
  }
...
template < typename T >
  std::vector < T > setDifference(const std::unordered_set < T > & setA,
...
ReturnValue Game::beforeCreatureZoneChange(std::shared_ptr < Creature > creature,
  const std::unordered_set < std::shared_ptr < Zone >> & fromZones,
    const std::unordered_set < std::shared_ptr < Zone >> & toZones, bool force /* = false*/ ) const {
  if (!creature) {
    return RETURNVALUE_NOTPOSSIBLE;
  }
...
void Game::afterCreatureZoneChange(std::shared_ptr < Creature > creature,
  const std::unordered_set < std::shared_ptr < Zone >> & fromZones,
    const std::unordered_set < std::shared_ptr < Zone >> & toZones) const {
  if (!creature) {
    return;
  }
