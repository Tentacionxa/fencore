...
ReturnValue internalMoveItem(std::shared_ptr<Cylinder> fromCylinder, std::shared_ptr<Cylinder> toCylinder, int32_t index, std::shared_ptr<Item> item, uint32_t count, std::shared_ptr<Item>* movedItem, uint32_t flags = 0, std::shared_ptr<Creature> actor = nullptr, std::shared_ptr<Item> tradeItem = nullptr, bool checkTile = true);
	...
	void playerLootAllCorpses(std::shared_ptr<Player> player, const Position &pos, bool lootAllCorpses, bool autoloot);
	...
	void addMagicEffect(const std::vector<std::shared_ptr<Creature>> &spectators, const Position &pos, uint16_t effect);
	...
	void addDistanceEffect(const std::vector<std::shared_ptr<Creature>> &spectators, const Position &fromPos, const Position &toPos, uint16_t effect);
